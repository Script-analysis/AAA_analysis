setwd("C:\\AAA_analysis\\sAAA_lAAA\\step7.")
library(GEOquery)
library(dplyr)
library(stringr)
library(tidyverse)
gse<-getGEO('GSE98278',destdir = '.',AnnotGPL = T,getGPL = T) 
...
library(dplyr)
clin <- clin %>%filter(group %in% c("sAAA", "lAAA"))
samples <- clin$Sample
...
write.table(data_sub,file="Rawdata_GSE98278.txt",sep="\t",quote=F,col.names=T,row.names = T)
write.table(clin_sub,file="clin_GSE98278.txt",sep="\t",quote=F,col.names=T,row.names = T)
clin<-read.xlsx("Meta.xlsx")
...
dev.off()
write.table(expr1_norm,file="GSE98278_normalization.txt",sep="\t",quote=F,col.names=T,row.names = T)
